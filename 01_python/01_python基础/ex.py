...
def calcInformationGain(dataSet,baseEntropy,i):
    ...
    uniqueVals = set(featList) # 取出有几种特征值- 集合中的每个元素不重复
...
def createTree(dataSet,featureName,chooseBestFeatureToSplitFunc = chooseBestFeatureToSplitByID3):
    ...
    myTree ={bestFeatLabel:{}}  # #创建字典树，map 结构，且key为featureLabel
    del (featureName[bestFeat])  #删除已经使用的标签
    # 找到需要分类的特征子集
...
def createPlot(inTree):
    ...
    # frameon表示是否绘制坐标轴矩形
    createPlot.ax1 = plt.subplot(111, frameon=False, **axprops)  # 设置一个多图展示，但是设置多图只有一个
